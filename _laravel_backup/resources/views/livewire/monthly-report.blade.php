<div>
    <div class="mb-8">
        <h2 class="text-2xl font-bold text-primary">{{ __('Laporan Bulanan Automasi') }}</h2>
        <p class="text-base-content/70 text-sm">{{ __('Kelola & unduh hasil tarikan data DTTO/PEP bulanan untuk semua cabang.') }}</p>
    </div>
    ...
    <div class="card bg-base-100 shadow-sm border border-base-200 mb-6 rounded-2xl">
        <div class="card-body p-5">
            <div class="flex items-center gap-2 mb-4">
                ...
                <h3 class="font-bold text-base text-base-content">{{ __('Generate Laporan Baru') }}</h3>
            </div>
            
            <form wire:submit.prevent="generateReport">
                <div class="flex flex-col sm:flex-row items-end gap-4">
                    <div class="form-control w-full sm:w-56">
                        <label class="label pb-1"><span class="label-text text-xs font-bold text-base-content/60 uppercase tracking-wide">{{ __('Pilih Bulan') }}</span></label>
                    ...
                    <div class="form-control w-full sm:w-48">
                        <label class="label pb-1"><span class="label-text text-xs font-bold text-base-content/60 uppercase tracking-wide">{{ __('Pilih Tahun') }}</span></label>
    ...
    <div class="card bg-base-100 shadow-sm border border-base-200 rounded-2xl overflow-hidden">
        <div class="flex flex-row flex-wrap items-center justify-between px-6 py-4 border-b border-base-200 bg-base-100">
            <div class="flex items-center gap-2">
                ...
                <h3 class="font-bold text-base text-base-content">{{ __('Daftar Laporan Tersedia') }}</h3>
            </div>
            <div class="text-xs text-base-content/60 font-semibold bg-base-200 px-3 py-1 rounded-full">
                {{ count($reports) }} {{ __('File Tersedia') }}
            </div>
        </div>
            ...
            <table class="table w-full text-sm">
                <thead>
                    <tr class="bg-base-200/50 border-b border-base-200">
                        <th class="px-6 py-3 text-xs font-bold text-base-content/60 uppercase tracking-wide">{{ __('Periode') }}</th>
                        <th class="px-6 py-3 text-xs font-bold text-base-content/60 uppercase tracking-wide">{{ __('Nama Berkas') }}</th>
                        <th class="px-6 py-3 text-xs font-bold text-base-content/60 uppercase tracking-wide">{{ __('Dibuat Pada') }}</th>
                        <th class="px-6 py-3 text-xs font-bold text-base-content/60 uppercase tracking-wide">{{ __('Ukuran') }}</th>
                        <th class="px-6 py-3 text-xs font-bold text-base-content/60 uppercase tracking-wide text-right">{{ __('Aksi') }}</th>
                    </tr>
                </thead>
                    ...
                    @forelse($reports as $report)
                        <tr class="hover:bg-base-200/40 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <button wire:click="downloadReport('{{ $report['year'] }}', '{{ $report['month'] }}')" class="btn btn-sm btn-success text-white font-medium hover:scale-105 transition-transform gap-1.5 shadow-sm">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-3.5 h-3.5">
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center justify-center opacity-40">
                                    ...
                                    <p class="text-sm font-semibold">{{ __('Belum ada laporan tersedia') }}</p>
                                    <p class="text-xs mt-1">{{ __('Silakan generate laporan baru terlebih dahulu.') }}</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
