<div>
    <div class="mb-6 flex flex-col gap-1">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-xl font-bold text-base-content">{{ __('Detail Informasi Terduga') }}</h2>
        </div>
    </div>
    ...
    @if($terduga->is_pending)
        <div class="alert alert-warning shadow-sm mb-6 max-w-5xl rounded-xl border-l-4 border-l-warning">
            ...
            <span><strong>{{ __('Menunggu Persetujuan:') }}</strong> {{ __('Data ini sedang dalam proses review berjenjang dan belum dipublikasikan secara final.') }}</span>
        </div>
    @endif
                    ...
                    <button x-data="{
                        confirmDelete() {
                            Swal.fire({
                                ...
                                text: '{{ __('Apakah Anda yakin ingin mengajukan penghapusan subjek ini?') }}',
                                icon: 'warning',
        ...
        <div class="card-body p-6 md:p-8">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-10">
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <div class="text-xs text-base-content/50 mb-1">{{ __('Tempat Lahir') }}</div>
                            <div class="font-medium text-base-content">{{ $terduga->tempat_lahir ?: '-' }}</div>
                        </div>
                        <div>
                            <div class="text-xs text-base-content/50 mb-1">{{ __('Tanggal Lahir') }}</div>
                            <div class="font-medium text-base-content">{{ $terduga->tanggal_lahir ? $terduga->tanggal_lahir : '-' }}</div>
                        </div>
                        <div>
                            <div class="text-xs text-base-content/50 mb-1">{{ __('Warga Negara') }}</div>
                            <div class="font-medium text-base-content">{{ $terduga->wn_asal_negara ?: '-' }}</div>
                        </div>
                        <div>
                            <div class="text-xs text-base-content/50 mb-1">{{ __('Tanggal Input Data') }}</div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-base-200/50 p-4 rounded-xl text-sm leading-relaxed text-base-content border border-base-200">
                        {!! nl2br(e($terduga->alamat)) ?: '<em class="text-base-content/40">' . __('Tidak ada informasi alamat yang dicatat.') . '</em>' !!}
                    </div>
                </div>
            ...
            <div class="space-y-6 pt-6 border-t border-base-200">
                <h4 class="text-sm font-bold text-base-content/50 uppercase tracking-wider flex items-center gap-2">
                    ...
                    {{ __('Catatan & Deskripsi Kejahatan') }}
                </h4>
                
                <div class="bg-base-200/30 p-5 rounded-xl text-sm leading-relaxed text-base-content whitespace-pre-wrap font-sans border border-base-200">{!! nl2br(e($terduga->deskripsi)) ?: '<em class="text-base-content/40">' . __('Tidak ada deksripsi atau catatan keterlibatan khusus.') . '</em>' !!}</div>
            </div>
        </div>
    </div>
</div>
