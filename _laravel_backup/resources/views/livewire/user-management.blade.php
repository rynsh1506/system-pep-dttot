<div>
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
            <label class="input input-bordered flex items-center gap-2 w-full sm:w-64">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" class="grow" placeholder="Cari user..." />
            </label>
                        ...
                        <tr class="hover">
                            ...
                            <td class="text-center">
                                <div class="flex gap-1 justify-center">
                                    <button wire:click="openEdit({{ $user->id }})" class="btn btn-xs btn-warning gap-1">
    ...
    <div class="modal modal-open">
        <div class="modal-box w-full max-w-lg p-6 rounded-2xl shadow-xl">
            <h3 class="font-bold text-xl mb-6 text-base-content border-b border-base-200 pb-3">{{ $isEditing ? 'Edit User' : 'Tambah User Baru' }}</h3>
            ...
            <div class="flex flex-col gap-5">
                <div class="form-control w-full">
                    ...
                    <input wire:model="nama_lengkap" type="text" placeholder="Masukkan nama lengkap..." class="input input-bordered w-full focus:outline-primary @error('nama_lengkap') input-error @enderror" />
                    @error('nama_lengkap') <span class="text-error text-xs mt-1.5">{{ $message }}</span> @enderror
                </div>
                ...
                <div class="form-control w-full">
                    ...
                    <input wire:model="username" type="text" placeholder="Masukkan username untuk login..." class="input input-bordered w-full font-mono focus:outline-primary @error('username') input-error @enderror" />
                    @error('username') <span class="text-error text-xs mt-1.5">{{ $message }}</span> @enderror
                </div>
                ...
                <div class="form-control w-full">
                    <label class="label pb-2">
                        ...
                        @if($isEditing)
                            <span class="label-text-alt text-base-content/50 font-normal">(Kosongkan jika tidak ingin diubah)</span>
                        @endif
                    </label>
                    <input wire:model="password" type="password" placeholder="{{ $isEditing ? 'Password baru (opsional)' : 'Masukkan password (min 6 karakter)...' }}" class="input input-bordered w-full focus:outline-primary @error('password') input-error @enderror" />
                    @error('password') <span class="text-error text-xs mt-1.5">{{ $message }}</span> @enderror
                </div>
                ...
                <div class="form-control w-full">
                    ...
                    <select wire:model="level" class="select select-bordered w-full focus:outline-primary @error('level') select-error @enderror">
                    ...
                    @error('level') <span class="text-error text-xs mt-1.5">{{ $message }}</span> @enderror
                </div>
            </div>
