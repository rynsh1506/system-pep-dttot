<div>
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        ...
        <button wire:click="openCreate" class="btn btn-primary btn-sm gap-2 shrink-0">
    ...
    <div class="modal modal-open">
        <div class="modal-box w-full max-w-md">
            <h3 class="font-bold text-lg mb-4">{{ $isEditing ? 'Edit User' : 'Tambah User Baru' }}</h3>

            <div class="flex flex-col gap-3">
                <div class="form-control">
                    <label class="label pb-1"><span class="label-text font-semibold text-sm">Nama Lengkap</span></label>
                    <input wire:model="nama_lengkap" type="text" placeholder="Nama lengkap..." class="input input-bordered input-sm @error('nama_lengkap') input-error @enderror" />
                    @error('nama_lengkap') <span class="text-error text-xs mt-1">{{ $message }}</span> @enderror
                </div>

                <div class="form-control">
                    <label class="label pb-1"><span class="label-text font-semibold text-sm">Username</span></label>
                    <input wire:model="username" type="text" placeholder="Username untuk login..." class="input input-bordered input-sm font-mono @error('username') input-error @enderror" />
                    @error('username') <span class="text-error text-xs mt-1">{{ $message }}</span> @enderror
                </div>

                <div class="form-control">
                    <label class="label pb-1">
                        <span class="label-text font-semibold text-sm">Password {{ $isEditing ? '(kosongkan jika tidak diubah)' : '' }}</span>
                    </label>
                    <input wire:model="password" type="password" placeholder="{{ $isEditing ? 'Password baru (opsional)' : 'Password...' }}" class="input input-bordered input-sm @error('password') input-error @enderror" />
                    @error('password') <span class="text-error text-xs mt-1">{{ $message }}</span> @enderror
                </div>

                <div class="form-control">
                    <label class="label pb-1"><span class="label-text font-semibold text-sm">Role / Level</span></label>
                    <select wire:model="level" class="select select-bordered select-sm @error('level') select-error @enderror">
                    ...
                    @error('level') <span class="text-error text-xs mt-1">{{ $message }}</span> @enderror
                </div>
            </div>

            <div class="modal-action">
                <button wire:click="$set('showModal', false)" class="btn btn-ghost btn-sm">Batal</button>
