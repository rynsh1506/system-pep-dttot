        ...
        <p class="text-base-content/70 text-sm">
            @if(session('role_level') >= 3)
                <span class="text-success font-semibold flex items-center gap-1 inline-flex">
                    ...
                    {{ __('Bypass Mode:') }}
                </span> {{ __('Data akan langsung disimpan tanpa proses approval.') }}
            @else
                ...
                {{ __('Data akan diajukan ke proses Approval berjenjang.') }}</span>
            @endif
        </p>
    </div>
    ...
    <div class="card bg-base-100 shadow-sm border border-base-200 w-full max-w-5xl">
        <div class="card-body p-6 md:p-8">
            <form x-data="{
                confirmSubmit() {
                    Swal.fire({
                        title: '{{ __('Konfirmasi Simpan') }}',
                        text: '{{ __('Apakah Anda yakin ingin menyimpan data ini?') }}',
                        icon: 'question',
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
                    ...
                    <div class="form-control w-full">
                        <label class="label pb-1"><span class="label-text font-semibold text-base-content/80">{{ __('Nama Lengkap / Entitas') }} <span class="text-error">*</span></span></label>
                        <input type="text" wire:model="nama" class="input input-bordered w-full @error('nama') input-error @enderror" placeholder="{{ __('Contoh: Budi Santoso / PT. Fiktif') }}" />
                        @error('nama') <span class="text-error text-xs mt-1">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-control w-full">
                        <label class="label pb-1"><span class="label-text font-semibold text-base-content/80">{{ __('Tipe Terduga') }} <span class="text-error">*</span></span></label>
                        <select wire:model="terduga_type" class="select select-bordered w-full @error('terduga_type') select-error @enderror">
                            ...
                            <option value="Orang">{{ __('Individu (Orang)') }}</option>
                            <option value="Korporasi">{{ __('Organisasi / Korporasi') }}</option>
                        </select>
                    ...
                    {{-- Row 2 --}}
                    <div class="form-control w-full">
                        <label class="label pb-1"><span class="label-text font-semibold text-base-content/80">{{ __('Kode Densus') }}</span></label>
                        <input type="text" wire:model="kode_densus" class="input input-bordered w-full font-mono @error('kode_densus') input-error @enderror" placeholder="{{ __('Contoh: DNS-2023-XYZ') }}" />
                        @error('kode_densus') <span class="text-error text-xs mt-1">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-control w-full">
                        <label class="label pb-1"><span class="label-text font-semibold text-base-content/80">{{ __('WN / Asal Negara') }} <span class="text-error">*</span></span></label>
                        <input type="text" wire:model="wn_asal_negara" class="input input-bordered w-full @error('wn_asal_negara') input-error @enderror" placeholder="{{ __('Contoh: Indonesia') }}" />
                        @error('wn_asal_negara') <span class="text-error text-xs mt-1">{{ $message }}</span> @enderror
                    </div>

                    {{-- Row 3 --}}
                    <div class="form-control w-full">
                        <label class="label pb-1"><span class="label-text font-semibold text-base-content/80">{{ __('Tempat Lahir') }}</span></label>
                        <input type="text" wire:model="tempat_lahir" class="input input-bordered w-full @error('tempat_lahir') input-error @enderror" placeholder="{{ __('Contoh: Jakarta') }}" />
                        @error('tempat_lahir') <span class="text-error text-xs mt-1">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-control w-full">
                        <label class="label pb-1"><span class="label-text font-semibold text-base-content/80">{{ __('Tanggal Lahir / Berdiri') }}</span></label>
                        <div x-data x-init="flatpickr($refs.picker, { dateFormat: 'Y-m-d', locale: '{{ app()->getLocale() == 'ja' ? 'ja' : (app()->getLocale() == 'en' ? 'en' : 'id') }}' })">
                            <input x-ref="picker" type="text" wire:model="tanggal_lahir" class="input input-bordered w-full @error('tanggal_lahir') input-error @enderror" placeholder="{{ __('Pilih tanggal...') }}" />
                        </div>
                    ...
                    <div class="form-control w-full md:col-span-2 mt-2">
                        <label class="label pb-1"><span class="label-text font-semibold text-base-content/80">{{ __('Alamat Lengkap') }}</span></label>
                        <textarea wire:model="alamat" class="textarea textarea-bordered min-h-[120px] w-full text-sm leading-relaxed @error('alamat') textarea-error @enderror" placeholder="{{ __('Alamat lengkap terduga...') }}"></textarea>
                        @error('alamat') <span class="text-error text-xs mt-1">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-control w-full md:col-span-2">
                        <label class="label pb-1"><span class="label-text font-semibold text-base-content/80">{{ __('Deskripsi / Catatan Kejahatan') }}</span></label>
                        <textarea wire:model="deskripsi" class="textarea textarea-bordered min-h-[140px] w-full text-sm leading-relaxed @error('deskripsi') textarea-error @enderror" placeholder="{{ __('Detail kasus, alasan dimasukkan ke dalam daftar, atau catatan penting lainnya...') }}"></textarea>
                ...
                <div class="mt-10 pt-6 border-t border-base-200 flex flex-col-reverse sm:flex-row justify-end items-center gap-3">
                    <a href="{{ route('home') }}" class="btn btn-outline w-full sm:w-auto">{{ __('Batal') }}</a>
                    <button type="submit" class="btn btn-primary w-full sm:w-auto min-w-[160px] shadow-sm flex items-center justify-center">
                        <span wire:loading.remove wire:target="submit" class="flex items-center justify-center gap-2">
                            ...
                            {{ $terdugaId ? __('Simpan Perubahan') : __('Simpan Data Baru') }}
                        </span>
