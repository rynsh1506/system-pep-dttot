...
if (!function_exists('send_alert_email')) {
     ...
     */
    function send_alert_email($nama, $nik, $hasil_dtot, $hasil_pep, $source = 'Pengajuan Cek', $kontrak = '-') {
        ...
        $checked_by = session()->get('full_name') ?? session()->get('username') ?? 'Unknown';
        $waktu = date('d/m/Y H:i:s');

        $email->setSubject("⚠️ ALERT: Terindikasi $alert_type - $nama");

        $dtotColor = ($hasil_dtot === 'Terindikasi') ? '#e74a3b; font-weight:700;' : '#1cc88a;';
        $pepColor = ($hasil_pep === 'Terindikasi') ? '#e74a3b; font-weight:700;' : '#1cc88a;';
            ...
            <div style='background: #fff; border: 1px solid #e0e0e0; border-top: none; padding: 20px; border-radius: 0 0 8px 8px;'>
                ...
                <table style='width: 100%; border-collapse: collapse; font-size: 14px;'>
                    ...
                    <tr>
                        ...
                        <td style='padding: 8px 12px; border: 1px solid #eee; color: $dtotColor'>$hasil_dtot</td>
                    </tr>
                    <tr>
                        <td style='padding: 8px 12px; border: 1px solid #eee; font-weight: 600; background: #f8f9fc;'>Hasil PEP</td>
                        <td style='padding: 8px 12px; border: 1px solid #eee; color: $pepColor'>$hasil_pep</td>
                    </tr>
                    <tr>
        ...
        if ($email->send()) {
            ...
            log_message('error', 'Gagal kirim email alert: ' . $email->printDebugger(['headers']));
            return false;
        }
    }
}
