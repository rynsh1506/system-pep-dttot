...
<div>
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6 border-b border-base-200 pb-4">
        <div class="flex items-center gap-3">
            <a href="<?= route_to('dashboard') ?>" class="btn btn-sm btn-ghost btn-square">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
                    <path fill-rule="evenodd" d="M17 10a.75.75 0 0 1-.75.75H5.612l4.158 3.96a.75.75 0 1 1-1.04 1.08l-5.5-5.25a.75.75 0 0 1 0-1.08l5.5-5.25a.75.75 0 1 1 1.04 1.08L5.612 9.25H16.25A.75.75 0 0 1 17 10Z" clip-rule="evenodd" />
                </svg>
            </a>
            <div>
                <h1 class="text-xl font-bold text-base-content flex items-center gap-2">Tambah Data DTTOT Satuan</h1>
                <p class="text-sm text-base-content/50 mt-0.5">Tambah entri individu atau korporasi ke daftar DTTOT.</p>
            </div>
        </div>
    </div>
    ...
    <div class="card bg-base-100 border border-base-200 shadow-sm mb-6">
        <div class="card-body p-8">
            <form action="<?= route_to('terduga.store') ?>" method="POST">
                <?= csrf_field() ?>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div class="form-control w-full">
                        <label class="label"><span class="label-text font-bold text-base-content/90">Nama Lengkap / Korporasi</span></label>
                        <input type="text" name="nama" class="input input-bordered w-full focus:border-primary focus:outline-none" required>
                    </div>
                    <div class="form-control w-full">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="kode_densus" class="input input-bordered w-full font-mono text-sm focus:border-primary focus:outline-none">
                    </div>
                    <div class="form-control w-full">
                        <label class="label"><span class="label-text font-bold text-base-content/90">WN / Asal Negara</span></label>
                        <input type="text" name="wn_asal_negara" class="input input-bordered w-full focus:border-primary focus:outline-none" required>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div class="form-control w-full">
                        ...
                        <input type="text" name="tempat_lahir" class="input input-bordered w-full focus:border-primary focus:outline-none">
                    </div>
                    <div class="form-control w-full">
                        <label class="label"><span class="label-text font-bold text-base-content/90">Tanggal Lahir</span></label>
                        <input type="date" name="tanggal_lahir" class="input input-bordered w-full focus:border-primary focus:outline-none">
                    </div>
                </div>

                <div class="form-control w-full mb-6">
                    <label class="label"><span class="label-text font-bold text-base-content/90">Deskripsi / Keterangan</span></label>
                    <textarea name="deskripsi" class="textarea textarea-bordered w-full h-32 focus:border-primary focus:outline-none" required></textarea>
                </div>
                ...
                <div class="form-control w-full mb-10">
                    ...
                    <textarea name="alamat" class="textarea textarea-bordered w-full h-24 focus:border-primary focus:outline-none"></textarea>
                </div>
