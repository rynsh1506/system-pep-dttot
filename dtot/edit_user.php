...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if (!empty($_POST['password'])) {
            ...
            $stmt = $pdo_portal->prepare("UPDATE users SET nama_lengkap = ?, level = ?, password = ? WHERE id = ?");
            $stmt->execute([$full_name, $role, $password, $id]);
        } else {
            $stmt = $pdo_portal->prepare("UPDATE users SET nama_lengkap = ?, level = ? WHERE id = ?");
...
$stmt = $pdo_portal->prepare("SELECT * FROM users WHERE id = ?");
$stmt->execute([$id]);
$user = $stmt->fetch();
